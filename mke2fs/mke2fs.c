			    ...
			    char *progname)
{
	...
	while (1) {
		...
		if (*cp) {
			// TODO: Gaayab (This check should not be needed when we have eaten libext2fs)
...
// 	}
	...
	if (!fs_type) {
		printf("profile maangdis fstype\n");
		// profile_get_string(profile, "devices", device_name,
		// 		   "fs_type", 0, &fs_type);
	}
	if (!usage_types) {
		printf("profile maangdis usage_types\n");
		// profile_get_string(profile, "devices", device_name,
		// 		   "usage_types", 0, &usage_types);
	}
	if (!creator_os) {
		printf("profile maangdis creator os\n");
	...
	if (fs_param.s_rev_level != EXT2_GOOD_OLD_REV) {
		tmp = "sparse_super,large_file,filetype,resize_inode,dir_index";
		// TODO: Gaayab
		...
		for (cpp = fs_types; *cpp; cpp++) {
			if(strcmp(*cpp, "ext3") == 0) {
				tmp = "has_journal";
				edit_feature(tmp, &fs_param.s_feature_compat);
			}else if(strcmp(*cpp, "ext4") == 0) {
				tmp = "has_journal,extent,huge_file,flex_bg,metadata_csum,64bit,dir_nlink,extra_isize";
				edit_feature(tmp, &fs_param.s_feature_compat);
			}
			// TODO: Gaayab
			// tmp = NULL;
	...
	// TODO: Gaayab, upar wale ko check karo
	...
	// TODO: Gaayab, this should never happen, see above
	// if (inode_size == 0)
...
static errcode_t set_error_behavior(ext2_filsys fs)
{
	...
	// TODO: Gaayab arg = get_string_from_profile(fs_types, "errors", NULL);
	arg = NULL;
	if (arg == NULL)
		goto try_user;
...
no_journal:
	...
	// TODO: Gaayab, I don't need hugefiles
